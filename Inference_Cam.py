...
class Inference_Class():
    ...
    def inference_frame(self, opencv_frame):
        ...
        image_tensor = image_tensor.unsqueeze(0) # 4차원 텐서로 변경
        image_tensor = image_tensor.to(self.DEVICE)

        inference_result = self.model(image_tensor)
        inference_result = inference_result.squeeze() # 다시 3차원 텐서로 변경
        inference_result = inference_result.cpu().numpy() # [0.1, 0.8, 0.05, 0.05] 와 같은 형상

        result_frame = np.copy(opencv_frame)
        # np.argmax: 입력된 데이터에서 가장 큰 값의 인덱스 반환
        # label_map: 인덱스로 클래스 이름 반환
        ...
        result_frame = cv2.putText(
            result_frame, label_text, (10, 50), cv2.FONT_HERSHEY_PLAIN, fontScale=2.0,
    ...
    def inference_image(self, opencv_image):
        ...
        image_tensor = image_tensor.to(self.DEVICE)
        with torch.no_grad():
            inference_reesult = self.model(image_tensor)

        inference_reesult = inference_reesult.squeeze()
        inference_reesult = inference_reesult.cpu().numpy()
        result_frame = np.copy(opencv_image)
        label_text = self.label_map[np.argmax(inference_reesult)]
        class_prob = str(inference_reesult[np.argmax(inference_reesult)])
        result_frame = cv2.putText(
            result_frame, label_text + " " + class_prob, (10, 50), cv2.FONT_HERSHEY_PLAIN,
